...
class MonoJetTreeMakerPurity : public edm::EDAnalyzer {
    public:
        explicit MonoJetTreeMakerPurity(const edm::ParameterSet&);
        ~MonoJetTreeMakerPurity();
  ...
  void findFirstNonPhotonMother(const reco::Candidate*, int &, double &, double &,double &, double &);
  ...
  edm::EDGetTokenT<pat::PhotonRefVector> photonsLiviaToken;
  edm::EDGetTokenT<pat::PhotonRefVector> tightphotonsLiviaToken;
  ...
  int32_t nphotonsLivia;
  double phPHiso,  phCHiso, phLiviapt, phLiviaeta, phLiviaphi,phLiviaPHiso,phLiviaRND04PHiso,phLiviaRND08PHiso,phLiviaCHiso,phLiviaNHiso,phLiviasieie, phLiviahoe, phLiviaElectronVeto, phLiviaEA,phLiviaEAEGamma;
  ...
  double  rho,  pfmet, pfmetphi, t1pfmet, t1pfmetphi, pfmupt, pfmuphi, mumet, mumetphi, t1mumet, t1mumetphi, elmet, elmetphi, t1elmet, t1elmetphi, phmet, phmetphi, t1phmet, t1phmetphi;
  ...
  double   wzmass, wzmt, wzpt, wzeta, wzphi,wzmothid, l1pt, l1eta, l1phi, l2pt, l2eta, l2phi, parpt, pareta, parphi, ancpt, anceta, ancphi,ancmass;
  double   mu1pt, mu1eta, mu1phi, mu1pfpt, mu1pfeta, mu1pfphi, mu1iso, mu2pt, mu2eta, mu2phi, mu2pfpt, mu2pfeta, mu2pfphi, mu2iso, el1pt, el1eta, el1phi, el2pt, el2eta, el2phi, phpt, pheta, phphi, phElectronVeto;
...
MonoJetTreeMakerPurity::MonoJetTreeMakerPurity(const edm::ParameterSet& iConfig): 
  ...
  triggerResultsTag(iConfig.getParameter<edm::InputTag>("triggerResults")),
    ...
    photonsLiviaToken(consumes<pat::PhotonRefVector> (iConfig.getParameter<edm::InputTag>("photonsLivia"))),
    tightphotonsLiviaToken(consumes<pat::PhotonRefVector> (iConfig.getParameter<edm::InputTag>("tightphotonsLivia"))), 
...
MonoJetTreeMakerPurity::~MonoJetTreeMakerPurity() {
}

void MonoJetTreeMakerPurity::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    using namespace edm;
    ...
    Handle<pat::PhotonRefVector> photonsLiviaH;
    iEvent.getByToken(photonsLiviaToken, photonsLiviaH);
    pat::PhotonRefVector photonsLivia = *photonsLiviaH;
    
    Handle<pat::PhotonRefVector> tightphotonsLiviaH;
    iEvent.getByToken(tightphotonsLiviaToken, tightphotonsLiviaH);
    pat::PhotonRefVector tightphotonsLivia = *tightphotonsLiviaH;
   ...
   // Trigger info
    ...
    int hardestPhotonLiviaIndex = -1;
    double hardestPhotonLiviaPt = 0.0;
    for (size_t i = 0; i < tightphotonsLivia.size(); i++) {
      if (tightphotonsLivia[i]->pt() > hardestPhotonLiviaPt) {
        hardestPhotonLiviaIndex = i;
        hardestPhotonLiviaPt = tightphotonsLivia[i]->pt();
      }
    }
...
// Photon information
    ...
    phLiviaphi    = 0.0;
    phLiviaPHiso    = 0.0;
    phLiviaRND04PHiso    = 0.0;
    phLiviaRND08PHiso    = 0.0;
    phLiviaCHiso    = 0.0;
    phLiviaNHiso    = 0.0;
    phLiviasieie    = 0.0;
    phLiviahoe    = 0.0;
    phLiviaElectronVeto    = 0;
    nphotonsLivia = photonsLiviaH->size();
    phLiviaEA = 0.;
    phLiviaEAEGamma = 0.;
	...
	phElectronVeto   = tightphotons[hardestPhotonIndex]->passElectronVeto();
	phPHiso = tightphotons[hardestPhotonIndex] ->photonIso();//-rho * getGammaEAForPhotonIso(tightphotons[hardestPhotonIndex]->eta());

    }
    if (hardestPhotonLiviaIndex >= 0) {
      phLiviapt    = tightphotonsLivia[hardestPhotonLiviaIndex]->pt();
      phLiviaeta   = tightphotonsLivia[hardestPhotonLiviaIndex]->eta();
      phLiviaphi   = tightphotonsLivia[hardestPhotonLiviaIndex]->phi();
      phLiviaPHiso =  (*photonPHisoH)[ tightphotonsLivia[hardestPhotonLiviaIndex]];
      phLiviaRND04PHiso   = (*rndgammaiso04H)[tightphotonsLivia[hardestPhotonLiviaIndex]];

      phLiviaRND08PHiso   = (*rndgammaiso08H)[tightphotonsLivia[hardestPhotonLiviaIndex]];

      phLiviaCHiso =(*photonCHisoH)[ tightphotonsLivia[hardestPhotonLiviaIndex]] -rho * getChargedHadronEAForPhotonIso(tightphotonsLivia[hardestPhotonLiviaIndex]->eta());     
      phLiviaNHiso =(*photonNHisoH)[ tightphotonsLivia[hardestPhotonLiviaIndex]] -rho * getNeutralHadronEAForPhotonIso(tightphotonsLivia[hardestPhotonLiviaIndex]->eta());     
      phCHiso = tightphotonsLivia[hardestPhotonLiviaIndex] ->chargedHadronIso()-rho * getChargedHadronEAForPhotonIso(tightphotonsLivia[hardestPhotonLiviaIndex]->eta());
      phLiviasieie = (*photonsieieH)[ tightphotonsLivia[hardestPhotonLiviaIndex]];
      phLiviaElectronVeto   = tightphotonsLivia[hardestPhotonLiviaIndex]->passElectronVeto();
      phLiviahoe = tightphotonsLivia[hardestPhotonLiviaIndex]->hadTowOverEm();
      phLiviaEAEGamma = getGammaEAForPhotonIso(tightphotonsLivia[hardestPhotonLiviaIndex]->eta());
      if(abs(tightphotonsLivia[hardestPhotonLiviaIndex]->eta())<0.9) phLiviaEA = 0.1271;
      if(abs(tightphotonsLivia[hardestPhotonLiviaIndex]->eta())>0.9 &&abs(tightphotonsLivia[hardestPhotonLiviaIndex]->eta())<1.4442 ) phLiviaEA = 0.1101;
    }
    ...
    if (isWorZMCSample && gensH.isValid()&& hardestPhotonLiviaIndex>-1) {
 ...
 for (View<GenParticle>::const_iterator gens_iter = gensH->begin(); gens_iter != gensH->end(); ++gens_iter) {
          if (gens_iter->pdgId() == 22 && gens_iter->status() == 1 ) {
            ...
            if( abs(ancid) <= 5 || abs(ancid)==2212){
              double dR = computeDR(&(*gens_iter),tightphotonsLivia[hardestPhotonLiviaIndex] );
              wzid   = gens_iter->pdgId();
              wzpt   = gens_iter->pt();
              wzeta  = gens_iter->eta();
              wzphi  = gens_iter->phi();
              wzmothid = gens_iter->mother(0)->pdgId();
              if(dR<0.3&&abs((tightphotonsLivia[hardestPhotonLiviaIndex]->pt()-gens_iter->pt())/tightphotonsLivia[hardestPhotonLiviaIndex]->pt()) < 0.5){
                ismatch=1;
		TLorentzVector* genpho = new TLorentzVector();
		genpho->SetPtEtaPhiM(wzpt, wzeta,wzphi,0);
		TLorentzVector* genpart = new TLorentzVector();
...
double MonoJetTreeMakerPurity::computeDR(const reco::Candidate *genPart,pat::PhotonRef phot){
  double dR = 999.;
...
void MonoJetTreeMakerPurity::beginJob() {
    ...
    tree->Branch("phElectronVeto"                , &phElectronVeto                , "phElectronVeto/D");
    tree->Branch("nphotonsLivia"             , &nphotonsLivia             , "nphotonsLivia/i");
    tree->Branch("rho"             , &rho             , "rho/D");
    tree->Branch("phPHiso"                , &phPHiso                , "phPHiso/D");
    tree->Branch("phCHiso"                , &phCHiso                , "phCHiso/D");
    tree->Branch("phLiviapt"                 , &phLiviapt                 , "phLiviapt/D");
    tree->Branch("pLiviaheta"                , &phLiviaeta                , "phLiviaeta/D");
    tree->Branch("phLiviaphi"                , &phLiviaphi                , "phLiviaphi/D");
    tree->Branch("phLiviaPHiso"                , &phLiviaPHiso                , "phLiviaPHiso/D");
    tree->Branch("phLiviaRND04PHiso"                , &phLiviaRND04PHiso                , "phLiviaRND04PHiso/D");
    tree->Branch("phLiviaRND08PHiso"                , &phLiviaRND08PHiso                , "phLiviaRND08PHiso/D");
    tree->Branch("phLiviaCHiso"                , &phLiviaCHiso                , "phLiviaCHiso/D");
    tree->Branch("phLiviaNHiso"                , &phLiviaNHiso                , "phLiviaNHiso/D");
    tree->Branch("phLiviasieie"                , &phLiviasieie                , "phLiviasieie/D");
    tree->Branch("phLiviahoe"                , &phLiviahoe                , "phLiviahoe/D");
    tree->Branch("phLiviaEA"                , &phLiviaEA                , "phLiviaEA/D");
    tree->Branch("phLiviaEAEGamma"                , &phLiviaEAEGamma                , "phLiviaEAEGamma/D");
    tree->Branch("phLiviaElectronVeto"                , &phLiviaElectronVeto                , "phLiviaElectronVeto/D");
...
void MonoJetTreeMakerPurity::endJob() {
}

void MonoJetTreeMakerPurity::beginRun(edm::Run const& iRun, edm::EventSetup const& iSetup) {
...
void MonoJetTreeMakerPurity::endRun(edm::Run const&, edm::EventSetup const&) {
}

void MonoJetTreeMakerPurity::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {
}

void MonoJetTreeMakerPurity::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {
}
...
*/
void MonoJetTreeMakerPurity::findFirstNonPhotonMother(const reco::Candidate *particle, int& ancestorid, double& ancestorpt, double& ancestoreta, double& ancestorphi, double& ancestormass) {
    if (particle == 0) {
        return;
    }
    if (abs(particle->pdgId()) == 22) {
      findFirstNonPhotonMother(particle->mother(0), ancestorid, ancestorpt, ancestoreta, ancestorphi, ancestormass);
        ...
        ancestorphi = particle->mass();
    }
    return;
}

void MonoJetTreeMakerPurity::findMother(const reco::Candidate *particle, int& ancestorid, double& ancestorpt, double& ancestoreta, double& ancestorphi) {
    if (particle == 0) {
        return;
    }
    if (abs(particle->pdgId()) == 22) {
...
double MonoJetTreeMakerPurity::computeMuonIso(const reco::Muon& mu) {
...
void MonoJetTreeMakerPurity::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
    edm::ParameterSetDescription desc;
    desc.setUnknown();
    descriptions.addDefault(desc);
}
double MonoJetTreeMakerPurity::getChargedHadronEAForPhotonIso(double eta) {
...
double MonoJetTreeMakerPurity::getNeutralHadronEAForPhotonIso(double eta) {
...
double MonoJetTreeMakerPurity::getGammaEAForPhotonIso(double eta) {
...
double MonoJetTreeMakerPurity::getGammaNewEAForPhotonIso(double eta) {
...
DEFINE_FWK_MODULE(MonoJetTreeMakerPurity);
